...
if [[ ! -d "${input_path}" ]]; then
	exit_with_message "path \"${input_path}\" doesn't exist"
fi

mkdir -p "${output_dir}"

find "${input_path}" -type f \( -iname "*.deb" -o -executable \) | while read -r file; do
	...
	if [[ "${file}" == *.deb ]]; then
		...
		# Handle raw binary
		binfile=$(readlink -f "${file}")
		name=$(basename "${binfile}")
		version="unknown"
		#arch=$(uname -m)   # or detect via 'file'
		arch=$(file -b "${binfile}" | sed -n 's/.*\(x86-64\|ARM\|aarch64\|Intel 80386\).*/\1/p')
		arch=${arch:-unknown}
	fi
